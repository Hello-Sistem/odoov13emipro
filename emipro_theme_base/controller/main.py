...
class EmiproThemeBase(http.Controller):
    ...
    def Brand(self, brand=None, page=0, category=None, search='', ppg=False, **post):
        ...
        products = Product.search(domain, limit=ppg, offset=pager['offset'], order=WebsiteSale._get_search_order(self,post))
    ...
    def get_new_product_data(self, **kwargs):
        ...
        style_id = kwargs.get('style_id', False)
        products = request.env['product.template'].sudo().search([('website_published', '=', True)],order='id desc',limit=10)
        if products:
            values = {
                'filter_data': products,
                'is_default': False,
    ...
    def get_category_products_data(self, **kwargs):
        ...
        if is_discount == 'true':
            for product in without_discount_products:
                ...
                combination_info = product._get_combination_info(combination, add_qty=1, pricelist=pricelist if pricelist else False)
